...
use crate::storage::{
    get_active_option, list_expired_active_options, release_locked_to_recipient, unlock_collateral,
    update_active_option, ActiveOption, ActiveOptionStatus, OptionType,
};
...
pub async fn settle_single_option(
    ...
    if payout_to_buyer > 0 {
        ...
        .await
        {
            ic_cdk::println!("settle: buyer transfer failed: {:?}", e);
