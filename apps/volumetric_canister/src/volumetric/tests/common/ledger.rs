...
pub fn mint(env: &TestEnv, to: Account, amount: u64) -> Result<u64, TransferError> {
    transfer_with_dedup_fields(env, env.minter_canister, to, amount, None, None, None)
}
...
pub fn transfer(
    ...
    transfer_with_dedup_fields(env, from, to, amount, from_subaccount, None, None)
}

#[allow(dead_code)]
pub fn transfer_with_dedup_fields(
    env: &TestEnv,
    from: candid::Principal,
    to: Account,
    amount: u64,
