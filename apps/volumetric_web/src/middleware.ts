...
function generateCspHeaders(nonce: string): string {
  ...
  const scriptSrc = isDev
    ? `'self' 'unsafe-inline' 'unsafe-eval' ${TAWK_CSP_SOURCES}`
    : `'self' 'nonce-${nonce}' 'strict-dynamic' ${TAWK_CSP_SOURCES}`;

  const styleSrc = `'self' 'unsafe-inline' ${TAWK_CSP_SOURCES}`;

  const cspDirectives = [
    "default-src 'self'",
    `script-src ${scriptSrc}`,
    `style-src ${styleSrc}`,
    `img-src 'self' blob: data: https://cdn.jsdelivr.net ${TAWK_CSP_SOURCES}`,
    `font-src 'self' data: https://fonts.gstatic.com https://fonts.googleapis.com https://cdn.jsdelivr.net ${TAWK_CSP_SOURCES}`,
    `connect-src 'self' ${TAWK_CONNECT_CSP_SOURCES} https://mempool.space wss://mempool.space https://api.exchange.coinbase.com`,
    `frame-src 'self' ${TAWK_CSP_SOURCES}`,
