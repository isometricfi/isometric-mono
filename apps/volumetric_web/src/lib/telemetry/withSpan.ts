...
const tracer = trace.getTracer(WEB_APP_TRACER_NAME);
type SpanCallback<T> = (span: Span) => Promise<T> | T;

export function withSpan<T>(name: string, fn: SpanCallback<T>): Promise<T>;
export function withSpan<T>(name: string, attributes: Attributes, fn: SpanCallback<T>): Promise<T>;
